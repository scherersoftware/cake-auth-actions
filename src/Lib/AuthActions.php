...
class AuthActions
{
     ...
     * @return bool
     */
    public function isAuthorized(array $user, string $plugin, string $controller, string $action): bool
    {
        $isAuthorized = false;
     ...
     * @return bool
     */
    public function isPublicAction(string $plugin, string $controller, string $action): bool
    {
        if ($this->_options['camelizedControllerNames']) {
            $controller = Inflector::camelize($controller);
        } else {
